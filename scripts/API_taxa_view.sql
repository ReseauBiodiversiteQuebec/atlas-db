...
agg_ref AS (
  SELECT 
    ...
    json_agg(
      json_build_object(
        'source_name', source_name, 'valid_scientific_name', 
        valid_scientific_name, 'rank', rank, 
...
best_ref AS (
  SELECT 
    DISTINCT ON (id_taxa_obs) id_taxa_obs, 
...
vernacular_all AS (
  SELECT 
    v_lookup.id_taxa_obs, 
    taxa_vernacular.*, 
    COALESCE(taxa_vernacular_sources.source_priority, 9999) AS source_priority, 
    v_lookup.match_type, 
    COALESCE(taxa_vernacular.rank_order, -1) AS rank_order 
  FROM 
    taxa_obs_vernacular_lookup v_lookup 
  ...
  WHERE (v_lookup.match_type = ANY (ARRAY['exact'::text, 'partialexact'::text])) AND v_lookup.match_type <> 'complex'::text
    AND v_lookup.id_taxa_obs = ANY($1) 
  ORDER BY v_lookup.id_taxa_obs, v_lookup.match_type, taxa_vernacular.rank_order DESC, source_priority
), 
best_vernacular AS (
  SELECT 
    ver_en.id_taxa_obs, 
  ...
  FROM 
    ...
    LEFT JOIN (
      ...
      WHERE 
        language = 'fra'
    ) AS ver_fr ON ver_en.id_taxa_obs = ver_fr.id_taxa_obs
), 
vernacular_group AS (
  SELECT 
    ...
    json_agg(
      json_build_object(
        'name', vernacular_all.name, 'source', 
        vernacular_all.source_name, 'source_taxon_key', 
        vernacular_all.source_record_id, 
 ...
 WITH all_ref AS (
         ...
         SELECT DISTINCT ON (lu.id_taxa_obs, taxa_ref.rank) lu.id_taxa_obs,
           ...
           FROM taxa_obs taxa_obs_1,
            ...
            best_ref best_ref_1
          WHERE lu.id_taxa_obs = taxa_obs_1.id AND taxa_ref.id = lu.id_taxa_ref_valid AND lu.id_taxa_obs = best_ref_1.id_taxa_obs
          ORDER BY lu.id_taxa_obs, taxa_ref.rank
        ), full_taxonomy AS (
...
CREATE INDEX taxa_observed_scientific_name_idx
    ON api.taxa USING btree
    (observed_scientific_name COLLATE pg_catalog."default")
    TABLESPACE pg_default;
CREATE INDEX taxa_rank_idx
    ON api.taxa USING btree
    (rank COLLATE pg_catalog."default")
    TABLESPACE pg_default;
CREATE INDEX taxa_valid_scientific_name_idx
    ON api.taxa USING btree
    (valid_scientific_name COLLATE pg_catalog."default")
    TABLESPACE pg_default;
